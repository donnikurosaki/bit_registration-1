        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-6">
            ...
            <div>
                ...
                <div class="bg-white rounded-lg border border-gray-200 divide-y divide-gray-200">
                    ...
                    <div class="p-4 flex items-center justify-between">
                        ...
                        <div class="flex space-x-2">
                            <button @click.prevent="previewUrl='{{ route('admin.submissions.viewFile', ['submission' => $submission->id, 'fileType' => 'photo']) }}'; previewExt='{{ strtolower(pathinfo($submission->photo_path, PATHINFO_EXTENSION) ?: 'img') }}'; showPreview=true;" 
                                    class="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-md text-white bg-secondary hover:bg-secondary-dark focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-secondary">
                    ...
                    <!-- Bulletins -->
                    @if($submission->report_paths && count($submission->report_paths) > 0)
                    ...
                    @if($submission->additional_doc_paths && count($submission->additional_doc_paths) > 0)
            ...
            <form action="{{ route('admin.submissions.updateStatus', $submission) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select id="status" name="status" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-accent focus:border-accent sm:text-sm rounded-md">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="admin_notes" name="admin_notes" rows="4" class="shadow-sm focus:ring-accent focus:border-accent block w-full sm:text-sm border-gray-300 rounded-md">{{ $submission->admin_notes }}</textarea>
                    </div>
                </div>
            ...
            <div class="mt-2" >
                ...
                <template x-if="!['pdf','png','jpg','jpeg','gif','svg','webp','img'].includes(previewExt)">
                    <div class="text-center">
                        ...
                        <a :href="previewUrl" target="_blank" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-primary hover:bg-primary-dark focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary">
                            <svg class="h-5 w-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                            </svg>
